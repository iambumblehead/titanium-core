...
var RSAKeyFormat = module.exports = (function () {
...
*/
	...
	return {
	    read_block : function() {
		var size = binary.ba2i( value.slice(idx,idx+4) );
		idx+=4;
	...
	if ( typeof input == "string" ) {
	    return binary.base64x_decode( pack(  input  ) );
	} else {
	    return input;
	}
    }
	...
	var arr = [];
	arr = arr.concat( binary.i2ba( ksize ) );
	arr = arr.concat( binary.i2ba( bn.length ) );
	arr = arr.concat( bn );
	arr = arr.concat( binary.i2ba( be.length ) );
	arr = arr.concat( be );
	...
	arr = arr.concat( binary.i2ba(be.length) );
	arr = arr.concat( be );
	arr = arr.concat( binary.i2ba(bd.length) );
	arr = arr.concat( bd );
