...
function initRSA3( packages ) {
	...
	var _result = function() {
        ...
        return nonstruct.getNew( [ generator, _result, glob.EXIT ] ).ready().frequency(1).timeout(1).progress(progress).done(done).go();
    };
	...
	return nonstruct.getNew( [ closure, receiver, glob.EXIT ] ).ready().frequency(1).timeout(1).progress(progress).done(done).go();
    };
	...
	RSA.prototype.stepping_generate = function (B,E) {
	    ...
	    var self=this;
            ...
            self = nonstruct.getAugmented(self);
